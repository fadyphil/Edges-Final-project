...
class _Initial<T> implements ApiState<T> {
  const _Initial();
...
class _Loading<T> implements ApiState<T> {
  const _Loading();
...
class _Success<T> implements ApiState<T> {
...
class _Error<T> implements ApiState<T> {
...
@override
String toString() {
  return 'ApiState<$T>.error(message: $message)';
}
...
/// @nodoc
abstract mixin class _$ErrorCopyWith<T,$Res> implements $ApiStateCopyWith<T, $Res> {
